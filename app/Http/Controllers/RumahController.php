...
class RumahController extends Controller
{
    ...
    { 
        ...
        if (strlen($q) > 0) {
            $rumahs = Rumah::where('penghasilan_orangtua', 'LIKE', '%' . $q . '%')->get();
        } else {
            $rumahs = Rumah::get();
        }
    ...
    public function createRumah(Request $request)
    {
        $this->validate($request, [
            'penghasilanOrangtua' => 'required|numeric|min:500000|max:5000000',
            'nilaiAkademik' => 'required|numeric|min:50|max:100',
        ...
        $rumah->poin_sertifikat = $request->usiaRumah;
        $rumah->penghasilan_tanah = $request->njopTanah;
